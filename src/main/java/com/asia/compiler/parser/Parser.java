...
public class Parser { //dodac mape nazwa zmiennej + typ
    ...
    public List<IntermediateObject> parse(String code) {
        ...
        langParser parser = prepareParser(lexer, intermediateObjectList);
    ...
    private langParser prepareParser(langLexer lexer, List<IntermediateObject> list) {
        langParser parser = new langParser(new CommonTokenStream(lexer));
        parser.removeErrorListeners();
        parser.addErrorListener(ThrowingErrorListener.INSTANCE);
        parser.setErrorHandler(new BailErrorStrategy());

        parser.addParseListener(new IOListener(list));
